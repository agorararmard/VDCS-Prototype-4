...
func Garble(circ CircuitMessage) GarbledMessage {
	//fmt.Println("Garbling", string(circ.ComID.CID))
...
func ReRand(gcm GarbledMessage, r Randomness) GarbledMessage {
	//fmt.Println("Rerandomizing ", string(gcm.GarbledCircuit.ComID.CID))
	one := new(big.Int).SetInt64(1)
...
func Evaluate(gc GarbledMessage) (result ResEval) {
	//fmt.Println("Evaluating ", string(gc.GarbledCircuit.ComID.CID))
	//Setting computation ID
	result.CID = gc.CID
