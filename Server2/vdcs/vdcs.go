...
func Garble(circ CircuitMessage) GarbledMessage {
	...
	for k, val := range circ.InputGates {
		...
		for key, value := range val.TruthTable {
			...
			outC1, outC2, err := elgamal.Encrypt(cryptoRand.Reader, &privOutput.PublicKey, outKey.WireLabel)
...
func Evaluate(gc GarbledMessage) (result ResEval) {
	...
	for _, val := range gc.InputGates {
		...
		for garbledKey := range val.GarbledValuesC1 {
			...
			outLabel, err := elgamal.Decrypt(privOutput, new(big.Int).SetBytes(val.GarbledValuesC1[garbledKey]), new(big.Int).SetBytes(val.GarbledValuesC2[garbledKey]))
